...
_Noreturn void RTCoreMain(void)
{
    ...
    printf("--------------------------------\r\n");
    printf("W5500_RTApp_MT3620_BareMetal DHCP Client\r\n");
    printf("App built on: " __DATE__ " " __TIME__ "\r\n");

    /* DHCP client Initialization */
    if(gWIZNETINFO.dhcp == NETINFO_DHCP)
    {
        DHCP_init(2, gDATABUF);
    ...
    while (1)
    {
        ...
        if(run_user_applications)
        {
          ...
          if ((ret = loopback_tcps(0, s0_Buf, 50000)) < 0) // TCP server loopback test
          {
#ifdef _MAIN_DEBUG_
          ...
          if ((ret = loopback_tcps(1, s1_Buf, 50001)) < 0) // TCP server loopback test
          {
#ifdef _MAIN_DEBUG_
            printf("SOCKET ERROR : %ld\r\n", ret);
#endif
